...
try {
    ...
    //1.批量删除产品
    if (!deleteRecords('product', $id)) throw new Exception('批量删除产品失败');

    //2.批量删除产品对应路由
    if (!deleteRecords('tb_rewrite_rules', $rule_id)) throw new Exception('批量删除产品路由失败');

    $id_log = implode(',', $id);
    $rule_id_log = implode(',', $rule_id);
    updatelogs("产品管理，批量删除产品，ID：{$id_log}；删除对应路由规则，ID：{$rule_id_log}"); //记录操作日志
