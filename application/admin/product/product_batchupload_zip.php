...
$path = count($data->path) == 0 ? '' : object_array($data->path[0]); //产品信息压缩包
...
try {
    ...
    foreach ($excelData as $key => $value) {
        if ($key > 1) {
            ...
            $price = empty($value[7]) ? 0.00 : (float)$value[7]; //产品价格
            $specifications = $value[8]; //规格
            $origin = $value[9]; //产地
            $keywords = $value[10]; //关键词
            $description = $value[11]; //描述
            $add_time = time(); //添加时间
            $seo_title = '';
            $seo_keywords = '';
            $seo_description = '';
            ...
            $sql = "insert into product (classid,title,thumbnail,photo_album,enclosure,content,price,specifications,origin,keywords,description,add_time,seo_title,seo_keywords,seo_description) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt = $link->prepare($sql);
            if (!$stmt) throw new Exception('添加产品，SQL准备失败: ' . $link->error);

            $stmt->bind_param("sssssssssssssss", $class_id, $title, $thumbnail, $photo_album, $enclosure, $content, $price, $specifications, $origin, $keywords, $description, $add_time, $seo_title, $seo_keywords, $seo_description);
            if (!$stmt->execute()) throw new Exception('添加产品，SQL执行失败: ' . $link->error);

            $record_id = $stmt->insert_id; //当前新增记录的ID
            $insert_success_id[] = $record_id;

            $class_id_ary = json_decode($class_id);
            $detail_template_id = getDetailTemplateId('product_class', $class_id_ary[0]); //获取第一个分类对应的详情模板ID
            ...
            if (!empty($route_page)) {
                ...
                if ($rule_id > 0) {
                    //更新产品路由规则ID，rule_id
                    updateRecord('product', [$record_id], ["id"], ["rule_id" => $rule_id]);
                }
            }
            ...
            if (count($field) > 0) {
                ...
                if (!$res_field_info) throw new Exception('添加产品，自定义字段添加失败！ ');
            }
        }
    }
    ...
    $updateTips = "产品管理，通过zip压缩包批量添加产品，ID：" . implode(',', $insert_success_id); //操作日志事件内容
...
function getFieldCustom(): array
{
    $field_custom = [];
    //sql查询语句
    $sql = "select * from field_custom where table_name='product'";
    $res = my_sql($sql);
    if ($res) {
...
//                    var_dump($temp . $fileEnclosure . $filename);
                ...
                if ($ele1 instanceof \PhpOffice\PhpWord\Element\TextRun) {
                    foreach ($ele1->getElements() as $ele2) {
                        if ($ele2 instanceof \PhpOffice\PhpWord\Element\Text) {
                            ...
                            $imageSrc = $img_dir . md5($ele2->getSource()) . "." . $ele2->getImageExtension();
...
function getTableNode($node): string
{
    ...
    foreach ($node->getRows() as $row) {
        ...
        foreach ($row->getCells() as $cell) {
            // 单元格文字
            $texts = $cell->getText();
            $text = count($texts) > 0 ? $texts[0]->getText() : '';

            // 单元格中可能包含文本、图片等，这里只处理文本
            $return .= '<td>' . htmlspecialchars($text) . '</td>';
        }
...
//            $return .= getTableNode($ele);
            if ($ele instanceof \PhpOffice\PhpWord\Element\Cell) {

                foreach ($ele->getElements() as $ele2) {
                    $return .= '<td>';
                    $return .= getTextNode($ele2);
                    $return .= '<td>';
                }
