...
try {
    ...
    // 1.删除产品分类
    if (!deleteRecord('product_class', [$id], ["id"])) throw new Exception('删除产品分类失败');

    //2.删除路由规则
    if (!deleteRecord('tb_rewrite_rules', [$rule_id], ["id"])) throw new Exception('删除产品分类路由失败');

    updatelogs("产品管理，删除产品分类，ID：{$id}；删除对应路由规则，ID：{$rule_id}"); //记录操作日志
