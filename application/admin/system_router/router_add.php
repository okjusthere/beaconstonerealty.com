...
// 验证必要字段
if (empty($router_path) || empty($router_name) || empty($component)) {
    echo $jsonData->jsonData(400, '缺少必要字段，请检查后再保存', []);
    exit;
}

try {
    $sql = "insert into tb_system_router (parent_id, router_level, router_path, router_name, component, router_icon, hidden, sort) values (?,?,?,?,?,?,?,?)";
    $stmt = $link->prepare($sql);
    if (!$stmt) throw new Exception('保存路由语句准备失败：' . $link->error);
    $stmt->bind_param("iisssssi", $parent_id, $router_level, $router_path, $router_name, $component, $router_icon, $hidden, $sort);
    if (!$stmt->execute()) throw new Exception('保存路由出错：' . $stmt->error);
