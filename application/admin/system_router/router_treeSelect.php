...
try {
    // 准备主查询SQL
    $sql = "SELECT id,parent_id,router_level,router_name,sort FROM tb_system_router WHERE `hidden`='1' ORDER BY router_level ASC, sort DESC, id ASC";
    $stmt = $link->prepare($sql);
    if (!$stmt) throw new Exception('查询列表准备失败：' . $link->error);
    if (!$stmt->execute()) throw new Exception('查询列表执行失败：' . $link->error);
    ...
    // 构建树形结构
    $obj['data'] = buildTree($routerData, $id_current);
 ...
 * @param int $currentId 当前ID，用于判断disabled
 * @return array 树形结构数据
 */
function buildTree($data, $currentId)
{
    $tree = [];
    ...
    for ($currentLevel = $maxLevel; $currentLevel > 0; $currentLevel--) {
        ...
        foreach ($levelMap[$currentLevel] as $id => $node) {
            ...
            if (isset($levelMap[$parentLevel][$parentId])) {
                ...
                // 构建子节点数据结构
                $childNode = buildNodeStructure($node, $currentId);
    ...
    if (isset($levelMap[0])) {
        ...
        foreach ($levelMap[0] as $rootNode) {
            $treeNode = buildNodeStructure($rootNode, $currentId);
 ...
 */
function buildNodeStructure($node, $currentId)
{
    $treeNode = [
        'value' => $node['id'],
        'label' => $node['router_name'],
        'disabled' => ($node['id'] == $currentId) ? true : false,
        'level' => $node['router_level']
    ];
