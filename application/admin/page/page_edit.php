...
try {
    ...
    if ($is_custom === '1' && (($is_custom !== $before_is_custom) || ($template_id !== $before_template_id))) {
        ...
        //判断伪静态链接是否唯一
        if (haveSameStaticUrl($rule_page)) {
    ...
    if ($before_template_id !== $template_id || $before_is_custom !== $is_custom || $before_static_url !== $rule_page) {
    ...
    if (!$stmt) throw new Exception('更新页面信息，数据库操作准备失败: ' . $link->error);


    // 绑定参数
    $stmt->bind_param("sssssssssi",
        $title, $description, $thumbnail, $photo_album,
        $content, $is_show, $seo_title, $seo_keywords, $seo_description,
        $id
    );
    ...
    // 3. 更新自定义字段
    if (count($field) > 0) {
        $stmt_field = $link->prepare("UPDATE field_info SET field_content = ? WHERE record_id = ? AND table_name = 'news' AND field_name = ?");
