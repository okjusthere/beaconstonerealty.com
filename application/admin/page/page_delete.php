...
try {
    ...
    // 1.删除文章
    if (!deleteRecord('news', [$id], ["id"])) throw new Exception('删除页面失败');

    //2.删除路由规则
    if (!deleteRecord('tb_rewrite_rules', [$rule_id], ["id"])) throw new Exception('删除页面路由失败');

    updatelogs("页面管理，删除页面，ID：{$id}；删除对应路由规则，ID：{$rule_id}"); //记录操作日志
