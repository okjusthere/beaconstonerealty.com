...
$enclosure = (isset($data->enclosure) && count($data->enclosure) > 0) ? json_encode($data->enclosure, JSON_UNESCAPED_UNICODE) : ''; //附件
$photo_album = (isset($data->photo_album) && count($data->photo_album) > 0) ? json_encode($data->photo_album, JSON_UNESCAPED_UNICODE) : ''; //图片相册
$content = $data->content ?? ''; //详情内容
$add_time = time(); //添加时间
if (isset($data->add_time) && !empty($data->add_time)) { //如果自定义了文章发布时间，按照自定义的存入，否则读取当前时间
    $add_time = strtotime($data->add_time);
}
$view = $data->view ?? 0; //文章阅读量
$seo_title = isset($data->seo_title) ? $data->seo_title : ''; //SEO标题
$seo_keywords = isset($data->seo_keywords) ? $data->seo_keywords : ''; //SEO关键词
$seo_description = isset($data->seo_description) ? $data->seo_description : ''; //SEO描述
$field = isset($data->field) ? $data->field : []; //文章表自定义字段记录的内容
...
try {
    ...
    // 2. 使用预处理语句插入文章表数据
    $sql = "INSERT INTO news (classid, title, url, keywords, description, thumbnail, enclosure, photo_album, content, add_time, is_show, is_top, paixu, view, rule_id, seo_title, seo_keywords, seo_description)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $link->prepare($sql);
    if (!$stmt) throw new Exception('发布文章，数据库操作准备失败: ' . $link->error);

    // 绑定参数
    $stmt->bind_param("ssssssssssssiiisss",
        $classid, $title, $url, $keywords, $description,
        $thumbnail, $enclosure, $photo_album, $content,
        $add_time, $is_show, $is_top, $paixu, $view, $rule_id,
        $seo_title, $seo_keywords, $seo_description
    );

    if (!$stmt->execute()) throw new Exception('文章信息添加失败: ' . $stmt->error);

    $r_id = $stmt->insert_id; //当前添加的文章ID

    // 3.处理自定义字段
    if (count($field) > 0) {
        $field_sql = "INSERT INTO field_info (table_name, record_id, field_name, field_content, add_time) VALUES (?, ?, ?, ?, ?)";
        // 准备插入字段信息的语句
        $stmt_field = $link->prepare($field_sql);
