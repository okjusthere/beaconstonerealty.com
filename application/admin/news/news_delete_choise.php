...
try {
    ...
    //1.批量删除文章
    if (!deleteRecords('news', $id)) throw new Exception('批量删除文章失败');

    //2.批量删除文章对应路由
    if (!deleteRecords('tb_rewrite_rules', $rule_id)) throw new Exception('批量删除文章路由失败');

    $id_log = implode(',', $id);
    $rule_id_log = implode(',', $rule_id);
    updatelogs("文章管理，批量删除文章，ID：{$id_log}；删除对应路由规则，ID：{$rule_id_log}"); //记录操作日志
