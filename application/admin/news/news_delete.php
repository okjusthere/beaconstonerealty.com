...
try {
    ...
    // 1.删除文章
    if (!deleteRecord('news', [$id], ["id"])) throw new Exception('删除文章失败');

    //2.删除路由规则
    if (!deleteRecord('tb_rewrite_rules', [$rule_id], ["id"])) throw new Exception('删除文章路由失败');

    updatelogs("文章管理，删除文章，ID：{$id}；删除对应路由规则，ID：{$rule_id}"); //记录操作日志
