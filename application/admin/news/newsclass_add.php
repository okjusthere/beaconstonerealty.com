...
// 初始化变量
$paixu = isset($data->paixu) ? (int)$data->paixu : 0; //分类排序
$parentid = isset($data->parentid) ? (int)$data->parentid : 0; //父级ID
...
$thumbnail = (isset($data->thumbnail) && is_array($data->thumbnail) && count($data->thumbnail)) > 0 ? json_encode($data->thumbnail, JSON_UNESCAPED_UNICODE) : ''; //缩略图
$banner = (isset($data->banner) && is_array($data->banner) && count($data->banner)) > 0 ? json_encode($data->banner, JSON_UNESCAPED_UNICODE) : ''; //通栏图片
$content = $data->content ?? ''; //分类详情内容
$add_time = time(); //添加时间
$is_show = isset($data->is_show) ? (int)$data->is_show : 2; //是否显示
$seo_title = $data->seo_title ?? ''; //SEO标题
$seo_keywords = $data->seo_keywords ?? ''; //SEO关键词
$seo_description = $data->seo_description ?? ''; //SEO描述
$show_type = isset($data->show_type) ? (int)$data->show_type : 1; //文章列表展现样式
...
try {
    ...
    // 1. 插入路由规则
    $rule_id = insertRewriteRules($list_page . time(), $template_id, $is_custom);
    if ($rule_id === 0) {
        throw new Exception('路由信息添加失败，请联系管理员！');
    }

    // 2. 插入文章分类
    $sql = "INSERT INTO news_class (parentid, title, description, thumbnail, banner, content, add_time, is_show, paixu,  detail_template_id, rule_id, seo_title, seo_keywords, seo_description, show_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("isssssssiiisssi",
        $parentid, $title, $description, $thumbnail, $banner,
        $content, $add_time, $is_show, $paixu,
        $detail_template_id, $rule_id, $seo_title, $seo_keywords,
        $seo_description, $show_type
    );
    ...
    $r_id = $stmt->insert_id; //当前添加的文章分类ID
    ...
    // 记录操作日志
    updatelogs("文章管理，添加文章分类，ID：" . $r_id);
