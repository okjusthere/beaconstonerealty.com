...
include_once '../../../myclass/Basic.php'; // 引用自定义类
...
// 获取分页参数
$current_page = isset($_GET['currentPage']) ? $basic::filterInt($_GET['currentPage']) : 1;
$page_size = isset($_GET['pageSize']) ? $basic::filterInt($_GET['pageSize']) : 10;
...
// 构建基础SQL和条件
$base_sql = "SELECT * FROM tb_logs";
$count_sql = "SELECT COUNT(*) AS total FROM tb_logs";
$where = '';
$params = [];
$types = '';
...
// 添加搜索条件
if (!empty($keywords)) {
    $where = " WHERE admin_name LIKE ? OR event LIKE ? OR ip_address LIKE ?";
...
// 查询分页数据
$limit_sql = " LIMIT ?, ?";
$page_params = [($current_page - 1) * $page_size, $page_size];
$page_types = 'ii';

$stmt = $link->prepare($base_sql . $where . " ORDER BY id DESC" . $limit_sql);
if ($stmt === false) {
    echo $jsonData->jsonData(500, '数据库查询失败: ' . $link->error, $obj);
    exit();
}
