...
try {
    ...
    if (!empty($id)) {
        ...
        if (is_array($id)) {
            ...
            $wParentid = " nc.parentid in({$strParentid})";
        } else {
            $id = \basic\Basic::filterInt($id);
            $wParentid = " nc.parentid=?";
            $params[] = $id;
            $types .= "i";
        }
        $where = " AND{$wParentid}";
    } else {
        $where = " AND nc.parentid=0";
    }

    $news_class_field = processTableField("id,parentid,title,description,thumbnail,banner,content", "nc"); //要获取的文章分类字段
    //sql查询语句
    $sql = "SELECT {$news_class_field},trr.static_url AS url FROM news_class nc INNER JOIN tb_rewrite_rules trr ON nc.rule_id = trr.id WHERE nc.is_delete=0 AND nc.is_show = 1{$where} ORDER BY nc.paixu DESC,nc.id ASC";
...
function getChild(mysqli $link, $id = 0)
{
    ...
    $sql = "SELECT nc.id FROM news_class nc INNER JOIN tb_rewrite_rules trr ON nc.rule_id = trr.id WHERE nc.is_delete=0 AND nc.is_show=1 AND nc.parentid = ?";
