...
$keywords = isset($_POST['keywords']) ? \basic\Basic::filterStr($_POST['keywords']) : ''; //文章搜索关键词
$displays = isset($_POST['displays']) ? \basic\Basic::filterInt(trim($_POST['displays'])) : 1; //每页显示的个数
$currentpage = isset($_POST['currentpage']) ? \basic\Basic::filterInt(trim($_POST['currentpage'])) : 1; //当前页码
...
try {
    ...
    if (!empty($id)) {
        ...
        if (is_array($id)) {
            ...
            $id = explode(',', eliminateID(implode(',', $id), 'news_class')); //过滤掉没有访问权限的文章分类id
            $wClassid = classidWhere($id, $params, $types);
        } else {
            $id = \basic\Basic::filterInt($id);
            $claid = eliminateID(getNewsClassChildID($link, $id) . $id, 'news_class'); //过滤掉没有访问权限的文章分类id
    ...
    //关键词搜索语句拼接
    if (!empty($keywords)) {
        $where .= " AND (n.title LIKE ? OR n.keywords LIKE ? OR n.description LIKE ? OR n.content LIKE ?)";
        $k = "%{$keywords}%";
        $types .= "ssss"; //绑定参数类型
    ...
    //sql查询语句
    $sql = "SELECT {$news_field},CASE WHEN n.url IS NULL OR n.url='' THEN trr.static_url ELSE n.url END AS url FROM news n INNER JOIN tb_rewrite_rules trr ON n.rule_id = trr.id WHERE n.classid IS NOT NULL AND n.is_delete=0 AND n.is_show=1{$where} ORDER BY n.is_top DESC,n.paixu DESC,n.id DESC";
