...
try {
    ...
    // 构建FIELD排序部分
    $field_order = "FIELD(n.id, " . implode(',', array_fill(0, count($id_ary), '?')) . ")";
    //sql查询语句
    ...
    while ($row = $result->fetch_assoc()) {
        $row["classid"] = empty($row["classid"]) ? [] : json_decode($row["classid"], true);
        $row["thumbnail"] = getThumbnailPath(object_array(json_decode($row["thumbnail"])));
        $row["enclosure"] = empty($row["enclosure"]) ? [] : getEnclosurePath(object_array(json_decode($row["enclosure"])));
