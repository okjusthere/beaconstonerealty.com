...
function newsShow1(newsAry) {
    let newsshow = "<div class='index_news1'><ul>";
...
function newsShow2(newsAry) {
    let newsshow = "<div class='index_news2'><ul>";
...
function showModal(a, b) {
    let modalElement = document.createElement("div"); // 创建一个<div>元素，用于显示弹出框
    modalElement.classList.add("modal"); // 添加CSS类名
    modalElement.innerHTML = "<div class='popup_tip_box'><div class='popup_tip_close1'></div><div class='popup_tipbg'><div class='popup_tip'><div class='popup_tip_close popup_tip_close2' onclick='closeModal()'>×</div><div class='popup_tip_txt'><span><i></i>" + b + "</span></div></div></div></div>";
    document.body.appendChild(modalElement); // 将弹出框<div>元素追加到<body>末尾

    if (a == "success") {
        $(".popup_tip_box").addClass("success").find(".popup_tip").addClass("fadeInUp2");
    }
    if (a == "error") {
        $(".popup_tip_box").addClass("error").find(".popup_tip").addClass("fadeInUp2");
    }
    if (a == "warning") {
        $(".popup_tip_box").addClass("warning").find(".popup_tip").addClass("fadeInUp2");
    }

    setTimeout(function () {
        modalElement.parentNode.removeChild(modalElement);
        $(".popup_tip").removeClass("fadeInUp2");
    }, 1200)
}
...
//获取链接中指定变量的参数
function getUrlParameter(keys) {//keys参数的关键字
    let url = window.location.href; //获取当前链接
    let new_reg = new RegExp("(^|[&?])" + keys + "=([^&]*)(&|$)");
    let new_index = url.indexOf("?");
    let new_str = url.substring(new_index, url.length);
    let new_r = new_str.match(new_reg);
    if (new_r != null) return decodeURI(new_r[2]);
    return "";
}
